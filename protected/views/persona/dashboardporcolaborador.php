<?php
	$persona = Persona::model()->find('nombre = "' . $_GET['colaborador'] . '"');
	...
	$this->menu=array(
		...
		array('label'=>'Volver', 'url'=>array('objetivoGeneral/dashboardporobjetivogeneral?og='.$_GET['og'])),
	);
...
where o.responsable_aid = ' . $persona->id . '
 group by p.nombre;';
...
$queryObjetivos = 'select p.nombre as persona, count(*) as cantidad from Objetivo o
					...
					where o.estatus_did = 2 && o.responsable_aid = ' . $persona->id . '
					group by p.nombre;';
$commandObjetivos = $connection->createCommand($queryObjetivos);
$objetivos = $commandObjetivos->queryAll();
...
$queryAvanceObjetivosPorDepartamento = 'select p.nombre as persona, count(*) as cantidad from Objetivo o
					inner join Persona p on p.id = o.responsable_aid
					where o.estatus_did = 1 && o.responsable_aid = ' . $persona->id . '
					group by p.nombre;';
